...
/**
 * @file
 * Default theme implementation to display a "Tableaux de bord" node.
 */
...
<article id="node-<?php print $node->nid; ?>" class="bloc-article <?php print $classes; ?>"<?php print $attributes; ?>>
  ...
  <div class="complement-titre">
    <?php if (!empty($field_auteurs)): ?>
      Par : <?php echo $field_auteurs[0]['safe_value']; ?> -
    <?php endif; ?>
    <?php echo $content['field_date_sortie_publication'][0]['#markup'] ?>
  </div>
  ...
  <div class="article">
    <div class="rte">
      <div class="content"<?php print $content_attributes; ?>>
          ...
          <?php if (!empty($field_visuel_accroche)): ?>
            <span class="illus-gauche">
              ...
              <?php if (!empty($field_copyright)): ?>
                <span class="credit">&copy; &nbsp;<?php echo $field_copyright[0]['safe_value']; ?></span>
              <?php endif; ?>
            </span>
        ...
        <?php if (!empty($field_synthese)): ?>
          <?php echo $field_synthese[0]['safe_value']; ?>
        <?php endif; ?>

        <?php if (!empty($field_reperes_franciliens)): ?>
          <h2>Quelques repères chiffrés</h2>
          <?php for ($i = 0; $i < $nb_repere; $i++) {
        ...
        <?php if (!empty($field_lien_telechargement)): ?>
          <p>
            <a href="<?php echo file_create_url($content['field_lien_telechargement']['#items'][0]['filename']); ?>"
               title="<?php echo $field_titre_telechargement[0]['safe_value']; ?>">
              <?php echo $field_titre_telechargement[0]['safe_value']; ?>
            </a>
          </p>
        <?php endif; ?>
